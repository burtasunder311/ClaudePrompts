...
When active, Claude Code adds safety instructions to commit/PR prompts and strips all attribution to avoid leaking internal model codenames, project names, or other Anthropic-internal information.

## Activation

- `CLAUDE_CODE_UNDERCOVER=1` — force ON (even in internal repos)
- **AUTO** (default): active UNLESS the repo remote matches the internal allowlist
- There is NO force-OFF — if not confident in an internal repo, stays undercover
